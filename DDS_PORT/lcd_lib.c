...
#define LCD_DMASK (1<<LCD_D4|1<<LCD_D5|1<<LCD_D6|1<<LCD_D7)
#define LCD_DMAP(d)(((d)&0x01)<<LCD_D4 | (((d)>>1)&0x01)<<LCD_D5 | (((d)>>2)&0x01)<<LCD_D6 | (((d)>>3)&0x01)<<LCD_D7)
...
{
	//4 bit part
	//LDP = (LDP&0xF0)|((ch&0xF0)>>4);
	LDP = (LDP & ~LCD_DMASK) | LCD_DMAP(ch >> 4); //hi part
	LCP |= 1 << LCD_RS;
	LCP |= 1 << LCD_E;
	_delay_ms(1);
	LCP &= ~(1 << LCD_E);
	LCP &= ~(1 << LCD_RS);
	_delay_ms(1);
	//LDP = (LDP&0xF0)|(ch&0x0F);
	LDP = (LDP & ~LCD_DMASK) | LCD_DMAP(ch); //lo part
	LCP |= 1 << LCD_RS;
	LCP |= 1 << LCD_E;
	_delay_ms(1);
	LCP &= ~(1 << LCD_E);
...
{
	//4 bit part
	LDP = (LDP & ~LCD_DMASK) | LCD_DMAP(cmd >> 4);	//hi part
	LCP |= (1 << LCD_E);
	_delay_ms(1);
	LCP &= ~(1 << LCD_E);
	_delay_ms(1);
	LDP = (LDP & ~LCD_DMASK) | LCD_DMAP(cmd); //lo part
	LCP |= (1 << LCD_E);
	_delay_ms(1);
	LCP &= ~(1 << LCD_E);
	_delay_ms(1);
...
void LCDinit(void) //Initializes LCD
{
	...
	//---------one------
	LDP = (LDP & ~LCD_DMASK) | (1 << LCD_D5 | 1 << LCD_D4); //4 bit mode
	LCP |= (1 << LCD_E);
	_delay_ms(1);
	LCP &= ~(1 << LCD_E);
	_delay_ms(1);
	...
	_delay_ms(1);
	//-------three-------------
	LDP = (LDP & ~LCD_DMASK) | (1 << LCD_D5); //4 bit mode
	LCP |= (1 << LCD_E);
	_delay_ms(1);
	LCP &= ~(1 << LCD_E);
	_delay_ms(1);
